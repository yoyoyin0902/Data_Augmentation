...
class DataAugmentForObjectDetection():
    def __init__(self, rotation_rate=0.5, max_rotation_angle=5,
                 crop_rate=0.5, shift_rate=0.5, change_light_rate=0.5,change_dark_rate=0.5,
                 add_noise_rate=0.5, flip_rate=0.5,
                 cutout_rate=0.5, cut_out_length=50, cut_out_holes=5, cut_out_threshold=0.5,
        ...
        self.change_dark_rate = change_dark_rate
    ...
    def _addNoise(self, img):
        ...
        return random_noise(img, mode='salt', seed=int(time.time()), clip=True) * 255
    ...
    def dataAugment(self, img, bboxes):
        ...
        while change_num < 1:  # 默认至少有一种数据增强生效
            ...
            if self.is_changeLight:
                if random.random() > self.change_dark_rate:  # 改变亮度
                    change_num += 1
                    img = self._changeLight(img)
